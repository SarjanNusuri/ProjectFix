<section id="product" class="py-16 px-16">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
    ...
    <div class="max-w-sm bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
      ...
      <div class="px-6 pb-6">
        <p class="text-[#c86d6d] font-semibold mt-2">{{ $item->price }}</p>
        <h2 class="text-lg font-semibold text-gray-800 mt-1">{{ $item->name }}</h2>
        <p class="text-sm text-gray-500 mt-1 truncate w-50 ">
          {{ $item->description }}
        </p>
        <!-- Tombol ditengah -->
        <div class="mt-4 flex justify-center">
          <a href="{{ route('user.produk',$item->slug) }}" class="bg-[#c86d6d]/90 text-white text-sm px-4 py-2 rounded-full hover:bg-[#b25b5b] transition">
            Lihat Produk
          </a>
        </div>
      </div>
